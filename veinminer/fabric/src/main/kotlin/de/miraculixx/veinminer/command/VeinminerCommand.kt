...
object VeinminerCommand {
    ...
    fun register() {
        CommandRegistrationCallback.EVENT.register { dispatcher, registry, _ ->
            de.miraculixx.veinminer.command.Permissions.install { src, node ->
            ...
            dispatcher.register(buildTree(registry))
        }
    }

    private fun buildTree(ctx: CommandBuildContext) = veinminerCommand<CommandSourceStack>("veinminer") {
        executesAsync {
            source.msg(
        ...
        literal("blocks") {
            ...
            literal("remove") {
                argument("block", BlockPredicateArgument.blockPredicate(ctx)) {
                    suggestStrings { ctx -> ctx.suggestFilteredList(ConfigManager.veinBlocksRaw) }
                    executesAsync {
                        val id = getRaw(3) ?: return@executesAsync source.msg("Block can not be null", cRed)
        ...
        literal("groups") {
            ...
            literal("list") {
                ...
                stringArg("group") {
                    suggestStrings { ConfigManager.groupsRaw.map { it.name } }
                    executesAsync {
                        val name = com.mojang.brigadier.arguments.StringArgumentType.getString(this, "group")
            ...
            literal("create") {
                stringArg("name") {
                    executesAsync {
                        val name = com.mojang.brigadier.arguments.StringArgumentType.getString(this, "name")
            ...
            literal("remove") {
                stringArg("group") {
                    suggestStrings { ConfigManager.groupsRaw.map { it.name } }
                    executesAsync {
                        val name = com.mojang.brigadier.arguments.StringArgumentType.getString(this, "group").lowercase()
        ...
        literal(name) {
            ...
            when (typeOf<T>()) {
                typeOf<Boolean>(), typeOf<Boolean?>() -> boolArg("$name-new") {
                    executesAsync {
                        val value = com.mojang.brigadier.arguments.BoolArgumentType.getBool(this, "$name-new") as T
                ...
                typeOf<Int>(), typeOf<Int?>() -> intArg("$name-new", min = 0) {
                    executesAsync {
                        val value = com.mojang.brigadier.arguments.IntegerArgumentType.getInteger(this, "$name-new") as T
                        consumer.invoke(value, this)
                        ConfigManager.save()
                        source.msg("$name set to $value", cGreen)
                    }
                ...
                typeOf<Double>(), typeOf<Double?>() -> doubleArg("$name-new", min = 0.0, max = 1.0) {
                    executesAsync {
                        val value = com.mojang.brigadier.arguments.DoubleArgumentType.getDouble(this, "$name-new") as T
                        consumer.invoke(value, this)
                        ConfigManager.save()
                        source.msg("$name set to $value", cGreen)
                    }
    ...
    private fun CommandNodeBuilder<CommandSourceStack>.overrides(
        ...
        literal("unset") {
            stringArg("key") {
                suggestStrings { ctx -> ctx.resolve().nonNullKeys() }
                executesAsync {
                    val key = com.mojang.brigadier.arguments.StringArgumentType.getString(this, "key")
